...
class Estudiante extends Controller
{
  ...
  public function index()
  {
    ...
    $verificar = $this->model->verificarPermiso($id_user, 'estudiante');
...
//  die();
    ...
    for ($i = 0; $i < count($data); $i++) {
      if ($data[$i]['estado'] == 1) {
        ...
        <button class ="btn btn-success" type="button"onclick="btnReingresarEstudiante(' . $data[$i]['id'] . ');" ><i class="fas fa-undo"></i></button>
        <div/>';
      }
  ...
  public function registrar()
  {
    ...
    if (empty ($ci) || empty ($nombre)) {
      ...
      if ($id == "") {
        $data = $this->model->registrarEstudiante($ci, $nombre);
