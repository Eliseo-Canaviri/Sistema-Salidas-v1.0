...
class Permisos extends Controller
{
    ...
    public function index()
    {
        ...
        $verificar = $this->model->verificarPermiso($id_user, 'permisos');
...
//  die();
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $data[$i]['acciones'] = '<div>
        <button class ="btn btn-primary" type="button"onclick="btnEditarPermisos(' . $data[$i]['id_permiso'] . ');"><i class="fas fa-edit"></i></button>
        <button class ="btn btn-danger" type="button"onclick="btnEliminarPermisos(' . $data[$i]['id_permiso'] . ');" ><i class="fas fa-trash-alt"></i></button>
        ...
        <button class ="btn btn-success" type="button"onclick="btnReingresarEstudiante(' . $data[$i]['id_permiso'] . ');" ><i class="fas fa-undo"></i></button>
        <div/>';
            }
    ...
    public function eliminar(int $id)
    {
        ...
        $data = $this->model->ElimiarPermisos($id);
        if ($data == 1) {
            $msg = array('msg' => 'Permiso Eliminado con Exito ☻', 'icono' => 'success');
        } else {
            $msg = array('msg' => 'Error al Eliminar Permiso ☻', 'icono' => 'error');
        }
        echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        die();
    }
